...
const courses = () => {
  ...
  const getPendngCourses = () => {
    apiService.get("courses/unapproved").then(function (response) {
      ...
      <section>
        <div className="my-6">
          ...
          {pending.length >= 1 ? (
            <div>
              {pending.map(
                (item: WorkspaceType, index: React.Key | null | undefined) => (
                  <div
                    key={index}
                    className="p-1 flex flex-wrap justify-between w-full"
                  >
                    <div className="border my-4  border-[#1E1E1E59] p-4 rounded-md flex justify-between">
                      {typeof item.thumbnail === "string" ? (
                        ...
                        <img
                          className="rounded-md w-1/2 h-32 object-cover shadow-[26px_0px_32.099998474121094px_0px_#FDC3324D]"
                      ...
                      <div className="pl-10 w-full">
                        ...
                        <Link href={`/admin/${item._id}?page=${item.type}`}>
                          <button className="p-2 px-6 rounded-sm bg-primary">
                            Publish
                          </button>
                        </Link>
                      </div>
