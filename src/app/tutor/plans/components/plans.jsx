  ...
  const plans = [
    {
      ...
      features: [
        ...
        {
          feature: "Free PDF, Image, and Video Course Upload",
          available: true,
        },
        {
          feature: "Can create Seminars, Conferences, and Workshops",
          available: true,
        },
    ...
    },
    {
      name: "Standard",
      price: {
        monthly: { amount: "8000", payment_id: "133951" },
        yearly: { amount: "8000", payment_id: "133952" },
      },
    ...
    },
    {
      name: "Enterprise",
      price: {
        monthly: { amount: "15000", payment_id: "133953" },
        yearly: { amount: "150000", payment_id: "133954" },
      },
    ...
    <>
      <div className="flex flex-col items-center mt-9">
        ...
        <p className="text-gray-600 text-center mx-auto max-w-4xl">
          Activate your free 5-day trial. No Contracts. You can always change
          your plan or cancel anytime with one click if the premium service
          isn't right for you.
        </p>
        ...
        <div className="flex items-center justify-center gap-4 mt-4 mb-8">
          ...
          <div
            ...
            className={` px-1 rounded-[20px] w-[60px] ${
              isYearly ? "text-right" : ""
            }  items-center justify-center bg-primary`}
          >
            <span
              className={`inline-block w-[15px] h-[15px] -mt-3 bg-white rounded-full `}
            ></span>
          </div>
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={` p-6 rounded-lg  ${
                plan.isPopular
                  ? "-mt-5 shadow-[0px_0px_5px_2px_rgb(0,0,0,0.7)]"
                  : "shadow-[0px_0px_4px_1px_rgb(0,0,0,0.5)]"
              }  shadow-[0px_0px_4px_1px_rgb(0,0,0,0.5)]
                                }`}
              style={{ maxWidth: "400px" }}
            >
              {plan.isPopular &&
                userProfile?.premiumPlan !== plan.name.toLowerCase() && (
                  <span className="text-[18px] text-red-400 capitalize w-full text-center font-semibold flex justify-center">
                    Most Popular
                  </span>
                )}
              <h3
                className={`text-xl font-medium flex items-center gap-3  uppercase mb-2 `}
              >
                {plan.name}{" "}
                {userProfile?.premiumPlan === plan.name.toLowerCase() && (
                  <span className="capitalize text-[15px] border-2 px-2 rounded-lg border-primary">
                    Your Current Plan
                  </span>
                )}
              </h3>
                  ...
                  <>
                    {isYearly
                      ? new Intl.NumberFormat("en-NG", {
                        ...
                        }).format(parseFloat(plan.price.yearly.amount))
                      : new Intl.NumberFormat("en-NG", {
                          style: "currency",
                          currency: "NGN",
                          minimumFractionDigits: 0,
                  ...
                  <li key={idx} className="flex items-center">
                    ...
                    >
                      {item.available ? (
                        ...
                        <span className="w-[20px] h-[20px] inline-flex items-center justify-center text-[17px] border border-black rounded-full">
                          <BsX />{" "}
                        </span>
                      )}
                    </span>
                    ...
                    <button
                      onClick={() => {
                        setPaymentInfo({
                          ...
                          plan: plan.name,
                          planId: isYearly
                            ? plan.price.yearly.payment_id
                            : plan.price.monthly.payment_id,
                        });
                        setOpen(true);
                      }}
