      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center">
          {advantages.map((advantage) => (
            <div key={advantage.id} className="flex flex-col items-start  md:items-center font-sans">
              <div className="flex flex-row ">
                <div className="flex items-center justify-center w-8 h-8 rounded-full bg-[#FEEDCD] text-primary text-lg font-bold mb-4 mr-4">
                  {advantage.id}
                </div>
                <div className="w-96">
                  <h3 className="text-xl font-bold mb-2">{advantage.title}</h3>
                  <p className="text-base text-gray-600">{advantage.description}</p>
                </div>
              </div>
            </div>
          ))}
