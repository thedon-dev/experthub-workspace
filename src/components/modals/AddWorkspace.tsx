...
import { CategoryType, WorkspaceType } from "@/types/CourseType";
...
const AddWorkspace = ({
  ...
  const handleModulesInputChange = (
    index: number,
    field: string,
  ...
  const handleBenefitsInputChange = (
    index: number,
    value: string | number | boolean
  ) => {
    ...
    if (type === `online`) {
      if (value) {
        ...
        const startDateIn = new Date(
          new Date(dayjs(startDate).toDate()).setHours(
            parseInt(hours),
            parseInt(minutes)
          )
        );
    ...
    try {
      ...
      const requiredFields = {
        ...
        ...(type === "online" && {
          ...
          ...(instant
            ? { startTime, endTime }
            : { days: days.filter((day: any) => day.checked).length }),
        }),
        ...(type === "video" && { videos }),
        ...(type === "pdf" && { pdf }),
      };
      ...
      const startDateTime =
        type === "video" || type === "pdf"
          ? dayjs()
          : dayjs.utc(
              `${dayjs(startDate || "").format(`YYYY-MM-DD`)}T${
                instant
                  ? startTime
                  : days.filter((day: any) => day.checked)[0]?.startTime
              }:00`
            );
      const endDateTime =
        type === "video" || type === "pdf"
          ? dayjs()
          : dayjs.utc(
              `${dayjs(endDate || "").format(`YYYY-MM-DD`)}T${
                instant
                  ? endTime
                  : days.filter((day: any) => day.checked)[0]?.endTime
              }:00`
            );
  ...
  const removeBenefits = (targetIndex: any) => {
    const newArray = benefits.filter(
      (item: any, index: any) => index !== targetIndex
    );
                      ...
                      <div>
                        ...
                        <div className="flex justify-between">
                          <div className="w-[48%]">
                            ...
                            <input
                              onChange={(e) =>
                                setCourseDuration(parseInt(e.target.value))
                              }
                              value={courseDuration}
                              type="number"
                              className="border rounded-md w-full border-[#1E1E1ED9] p-2 bg-transparent"
                                ...
                                {type === `online` && (
                                  <>
                                    ...
                                    <span className="text-orange-500 leading-3 font-thin text-[12px]">
                                      {process.env.NEXT_PUBLIC_MEETING_DURATION}
                                      min max for Live
                                    </span>
                                  </>
                                )}{" "}
                            ...
                            {type === "online" && (
                              <div className="flex items-center border-b border-slate-500 my-5 gap-3 px-4">
                                <button
                                  ...
                                  className={`font-medium py-1 px-5 rounded-t-lg duration-300  ${
                      ...
                      <div>
                        ...
                        <div className="my-5">
                          ...
                          <input
                            type="number"
                            onChange={(e) =>
                              setStriked(parseInt(e.target.value))
                            }
                            value={striked}
