...
const AssignTutor = ({ open, handleClick, workspace, getCourse }) => {
  ...
  const enroll = () => {
    setLoading(true);
    apiService
      .post(`workspace/assign/${workspace._id}`, {
        id: enrollStudent,
      })
  ...
  const unAssign = (id) => {
    // setLoading(true)
    apiService
      .post(`courses/assign/${course._id}`, {
        id: id,
      })
      .then(function (response) {
        console.log(response.data);
        ...
        <div className="fixed top-10 bottom-10 left-0 rounded-md right-0 lg:w-[50%] w-[90%] h-[80%] overflow-y-scroll mx-auto z-20 bg-[#F8F7F4]">
          ...
          <div className="lg:p-10 p-4">
            <div className="my-3">
              ...
              {course.assignedTutors?.length >= 1 && (
                <div className="mt-4">
                  <p className="text-sm font-medium">Assigned Tutors</p>
                  <div>
                    {course.assignedTutors.map((single, index) => (
                      <div
                        className="flex justify-between w-[40%] my-2"
                        key={index}
                      >
                        <img
                          src={
