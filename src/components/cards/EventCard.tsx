...
const EventCard = ({ event, action }: { event: CourseType; action: any }) => {
  ...
  const items: MenuProps["items"] = [
    {
      key: "1",
      label: <p onClick={() => setEdit(true)}>Edit Event</p>,
    },
    {
      key: "2",
      label: <p onClick={() => setDelete(true)}>Delete Event</p>,
    },
    {
      key: "3",
      label: <p onClick={() => setView(true)}>View Participants</p>,
    },
    {
      key: "4",
      label: <Share type="event" course={event} />,
    },
  ];

  const deleteCourse = async () => {
    apiService.delete(`/events/delete/${event._id}`).then(function (response) {
      // getCourse()
      action();
      api.open({
          ...
          <Link
            href={
              pathname.includes("admin")
                ? `/admin/${event._id}?page=event`
                : `/tutor/${event._id}?page=event`
            }
          >
            <button className="bg-primary ml-3 text-sm p-1 rounded-md">
              {event.mode}
            </button>
          </Link>
        </div>
        ...
        <div className="flex justify-between my-3">
          <div>
            <p className="text-xs my-1">
              Students {event.enrolledStudents.length}
            </p>
            <div className="flex ml-1">
              {event.enrolledStudents.slice(0, 6).map((event) => (
                <img
                  key={event._id}
                  src={
        ...
        <div className="text-center my-3">
          {event.type === "online" ? (
            <button
              ...
              className="bg-primary p-2 w-44 text-white"
            >
              Join Live{" "}
            </button>
          ) : (
            <button
              onClick={() => setOpen(true)}
              className="border border-[#1E1E1E] text-[#DC9F08] p-2 w-44 mx-auto"
            >
              View Details
            </button>
          )}
