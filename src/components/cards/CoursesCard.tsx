  ...
  return (
    <div className="p-2 w-full shadow-md my-3 rounded-md bg-white">
      ...
      <h3 className="font-medium my-3">{course.title}
        {pathname.includes("courses") ? <Link href={`/admin/${course.type}?page=${course._id}&view=true`}>
          <button className='bg-primary p-2 rounded-md'>
            {course.type === 'online' ? 'Join Live' : course.type}</button>
        </Link> : <button onClick={() => setOpen(true)} className='bg-primary p-2 rounded-md'>
          {course.type === 'online' ? 'Join Live' : course.type}</button>}  </h3>
